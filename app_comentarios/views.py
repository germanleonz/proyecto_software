...
def crear_comentario(request):
  ...
  if request.method =='POST':
    ...
    if form.is_valid():
      ...
      crearAccion(usuario,"El usuario %s hizo un comentario en la actividad %s" % (usuario.username, act.nombreact), fechaYHora)
...
def eliminar_comentario(request):
  if request.method == 'POST':
    ...
    if actividad.loginjefe == request.user or actividad.loginasignado == request.user or actividad.logincreador == request.user or comentario.loginusuario == request.user:
      fechaYHora = datetime.now().strftime("%Y-%m-%d %H:%M")
      usuario = request.user
      crearAccion(usuario,"El usuario %s elimino un comentario en la actividad %s" % (usuario.username, actividad.nombreact), fechaYHora)
